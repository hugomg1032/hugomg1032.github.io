...
// El mensaje que se muestra al principio o al final
function mensaje(cadena) {
	var lon = (canvas.width - (41 * cadena.length)) / 2;
	ctx.fillStyle = "red";
...
function gameOver() {
	...
	balasEnemigas_array = [];
	if( enemigosVivos == 0 ){
		mensaje("SOBREVIVISTE");
	}else{
		mensaje("REMOVIDO");
		knch= "/imagenes/canichefinal.jpg"
	}
	endGame = true;
...
function score() {
	...
	ctx.fillText("PUNTAJE: " + puntos, 10, 20);
	ctx.restore();
}
function municiones() {
	ctx.save();
	...
	ctx.font = "bold 20px Courier";
	ctx.fillText("MUNICIONES: " + municion, 10, 470);
	ctx.restore();
}
...
function checarBalas(){
	...
	if(municion == 0 && balas_array.length == 100 && balasArrayVal == 0 && enemigosVivos > 0){
		tecla[teclaEspacio] = false;
			alert("Has perdido la proteccion divina");
			gameOver();
	}
}
function pinta() {
